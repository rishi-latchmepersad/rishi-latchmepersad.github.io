...
We can see that the models are generally able to track the positions of the needles in the gauges well, even though they haven't been specifically trained on these gauges. The biggest reason for this is because of the size of the training dataset. I was able to assemble over 8000 images of a diverse array of gauges. In each image, I labelled the gauge face, the needle center and the needle tip. I then trained a CNN to learn the features that would enable it to first find the gauge face from the entire photograph, then to crop the photo to that gauge face, then to pass this cropped image to a second model that would find the needle center and tip in the cropped photo. Seems that it's holding up pretty well so far!
